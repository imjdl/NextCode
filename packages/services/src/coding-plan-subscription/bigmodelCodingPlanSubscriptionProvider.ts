...
export class BigModelCodingPlanSubscriptionProvider {
  ...
  /**
   * 闲时任务可用性：定制版改为本地判定，不再受服务端 `offPeak.enable_offpeak_task` 开关控制。
   * 判据与 mock 路径一致——只要有可用模型即视为可用；能否真正排队仍由服务端票据接口决定。
   */
  ...
  async getDynamicWorkflowClientConfig(_options?: {
    ...
    // 定制版：动态工作流开关改为本地判定（env 覆盖 → 默认），不再读服务端灰度快照。
    // 判据（normalize）与快照构造（resolve）仍留在 shared，这里不复述取值域。
  ...
  async getForceUpdateConfig(): Promise<ForceUpdateConfig | null> {
    // 定制版：不读取远端强制升级配置。返回 null 表示"无强制升级要求"，
    // 且桌面端的 force-update gate 已整体移除，调用链不存在。
    return null;
  }
