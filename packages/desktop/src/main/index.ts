...
// 定制版：desktopContextPrompt 由服务端灰度旗标控制，这里整体移除，恒为关闭。
// 该开关原本会让服务端往 agent 侧注入"桌面上下文提示词"，属于远程改写本地行为。
function resolveDesktopContextPromptEnabledForHost(): boolean {
  return false;
}

// 上游在首个 Host fork 前 await 一次灰度裁决（≤2s）。定制版没有远端旗标需要裁决，
// 保留同名函数但立即完成，避免改动 Host 启动流程的调用点。
...
app.whenReady().then(async () => {
  ...
  // 定制版：不再预热远端灰度请求（desktopContextPrompt 已整体移除）。
  ...
  // 定制版关闭官方渠道更新：release manifest 与强制升级配置都来自官方后端，
  // 定制客户端会被官方 minimalVersion 判定强制升级，自动更新成官方安装包并丢失定制改动。
  // 恢复官方更新行为时改为 false 并重新打包。
  const customBuildOfficialUpdatesDisabled = true;
  // 启动自动更新检查（后台执行，不阻塞主界面）
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 ZCode 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: ZCODE_PRODUCT_FLAVOR === "production" && !customBuildOfficialUpdatesDisabled,
  ...
  disposeRendererActionTraceIpc = registerRendererActionTraceIpc({
    // 定制版：不传远端旗标，行为追踪恒关闭；注册 IPC 只为让渲染层调用有响应。
  ...
  // 定制版：不初始化 ARMS / 遥测上报（ZCODE_ARMS_RUM_ENDPOINT 已固定为空且初始化已移除），
  // 因此这里既不等 SDK、也不注册任何上报器；纯本地能力（崩溃抓取等）不受影响。
  const customBuildTelemetryDisabled = true;
  if (!customBuildTelemetryDisabled && ZCODE_TELEMETRY_ENABLED && ZCODE_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: ZCODE_VERSION,
  ...
  if (!customBuildTelemetryDisabled) {
    registerDesktopZCodeDataSizeTelemetry({
      context: {
        appVersion: ZCODE_VERSION,
        armsEnv: mapZCodeEnvToArmsRumEnv(desktopRuntimeEnv),
  ...
  // 定制版：不请求远端强制升级配置，启动流程不受服务端控制。
  // 上游的 force-update gate（面向官方分发客户端的安全门）在定制版整体移除，
  // 因此这里不再有网络请求，也不再可能被服务端 minimalVersion 阻塞启动。
  const forceUpdateGuardResult = { blocked: false };
  logger.info("[force-update] 定制版构建不启用远端强制升级检查");
