...
/**
 * 定制版：官方更新渠道整体关闭。编译期常量，initAutoUpdater 直接短路，
 * 即使调用方误传 enabled:true 也无法启用（服务端不能用版本号替换本包）。
 */
const customBuildUpdatesDisabled = true;
...
function applyManifestUpdateProvider(_options: InitAutoUpdaterOptions): void {
  // 定制版：不再配置官方 release manifest provider（上游在此 setFeedURL 到服务端清单，
  // 服务端可用 minimalVersion/manifest 驱动客户端更新）。initAutoUpdater 已整体短路，
  // 本函数保留为显式空实现，避免残留任何指向官方更新源的 feed 配置。
  logger.info("[auto-update] manifest provider disabled in this custom build");
}
