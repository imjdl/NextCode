// 定制版：ARMS 上报整体移除，统一使用空实现（不 init、不发送）。
import armsRum from "./armsRumDisabled.js";
import {
...
function send(
  ...
  // 定制版硬关断：ZCODE_TELEMETRY_ENABLED 编译期为 false，这里直接短路，
  // 不再触碰 ARMS（既不构造 payload 也不调用 SDK）。
