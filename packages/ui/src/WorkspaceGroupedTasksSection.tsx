...
export function WorkspaceGroupedTasksSection({
  ...
  // 激活任务落在折叠分组时自动展开该组：从任意入口恢复旧任务后无需手动翻找分组，
  // 行级滚动揭示由 VirtualizedGroupTaskList 在组内容挂载后完成。
  const lastRevealExpandedTaskKeyRef = useRef<string | null>(null);
  useEffect(() => {
    if (!activeTaskId) {
      return;
    }
    const activeTaskKey = `${buildTaskWorkspaceKey(activeWorkspacePath, activeWorkspaceIdentity)}\u0000${activeTaskId}`;
    if (activeTaskKey === lastRevealExpandedTaskKeyRef.current) {
      return;
    }
    const containingNode = view.nodes.find((node) =>
      node.type === "group"
        ? node.tasks.some((task) => taskKey(task) === activeTaskKey)
        : node.type === "task" && taskKey(node.task) === activeTaskKey,
    );
    // 视图尚未包含该任务（列表仍在加载）时不记已处理，等视图更新后重试。
    if (!containingNode) {
      return;
    }
    lastRevealExpandedTaskKeyRef.current = activeTaskKey;
    if (containingNode.type !== "group") {
      return;
    }
    const groupId = containingNode.group.id;
