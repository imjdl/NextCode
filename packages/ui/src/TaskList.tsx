...
export const TaskList = memo(function TaskList({
  ...
  const lastAutoScrolledActiveTaskIdRef = useRef<string | null>(null);
  useEffect(() => {
    // 修复原因：激活较早创建的任务时列表停在原滚动位置，用户必须长时间手动下拉才能找到当前任务。
    // activeTaskId 变化后把所在行滚入可视区；nearest 保证已可见时不打断手动浏览，
    // 且每个激活值只滚一次——流式刷新触发的重渲不会把用户主动滚离的位置再拉回来。
    if (!isWorkspaceActive || !activeTaskId) {
      return;
    }
    if (lastAutoScrolledActiveTaskIdRef.current === activeTaskId) {
      return;
    }
    if (!taskLookup.some((task) => task.taskId === activeTaskId)) {
      return;
    }
    const activeRow = listRootRef.current?.querySelector(
      `[data-testid="${testId(TID_TASK_ITEM, activeTaskId)}"]`,
    );
    if (!activeRow) {
      return;
    }
    lastAutoScrolledActiveTaskIdRef.current = activeTaskId;
    activeRow.scrollIntoView({ block: "nearest", behavior: "smooth" });
  }, [activeTaskId, isWorkspaceActive, taskLookup]);
  ...
  return (
    <div ref={listRootRef} data-testid={TID_TASK_LIST} className={"flex flex-col gap-2"}>
      {renamingTaskId !== null ? (
        <TaskRenameDialog
          open
          value={renameDraft}
