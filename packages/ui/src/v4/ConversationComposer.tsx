...
// 提示词增强的固定改写指令：只改写、不回答，保持原文语言与关键信息（路径/标识符/@ 引用）。
const PROMPT_ENHANCE_SYSTEM = [
  "You are a prompt engineering assistant for an AI coding agent.",
  "Rewrite the user's draft prompt so the agent can act on it effectively:",
  "clarify the goal, add missing context or constraints when clearly implied,",
  "and structure it into concise steps when helpful.",
  "Preserve the original language, technical terms, file paths, code identifiers and any @-mentions.",
...
function ConversationComposerImpl({
  ...
  const enhanceModelSelection = useMemo(() => {
    ...
    return selection;
  }, [draftConfig, telemetryDraftConfig, snapshot]);
  const enhancePromptAvailable = Boolean(services?.zcodeAgentService);
  ...
  const handleEnhancePromptClick = useCallback(async () => {
    const agentService = services?.zcodeAgentService;
    const draftText = textRef.current.trim();
    const selection = enhanceModelSelection;
    if (!agentService || enhancingPrompt || !draftText || !selection || !workspacePath) {
      return;
    }
  ...
  const submitControlNode = useMemo(
    () => (
      <div className="flex min-w-0 items-center gap-1">
        ...
        {enhancePromptAvailable ? (
          <ControlHintTooltip title={intl.formatMessage({ id: "composer.promptEnhance.tooltip" })}>
            <Button
              type="button"
              variant="ghost"
              size="icon-md"
              ...
              onClick={() => {
                void handleEnhancePromptClick();
              }}
              data-testid="v4-composer-prompt-enhance"
              aria-label={intl.formatMessage({ id: "composer.promptEnhance.tooltip" })}
            ...
            >
              ...
              <span className="sr-only">
                {intl.formatMessage({ id: "composer.promptEnhance.tooltip" })}
              </span>
            </Button>
          </ControlHintTooltip>
        ) : null}
