...
| 手机端访问 | 桌面端内置面板：开启后**局域网内**手机扫码访问（不支持公网）+ 响应安全头（CSP）与手机端布局适配 |
...
- 手机端访问所用的 Web 服务补齐响应安全头：`nosniff`、`no-referrer`（二维码 URL 带 token）、
...
| 手机端访问（局域网） | 用户名旁的手机图标进入控制面板：开关服务、可选内网 IP、展示二维码，手机扫码即用；**仅在局域网内可达，不支持公网访问**；开启后图标变绿 |
...
流水线三阶段：准备运行资源（含随包远端资源与手机端访问所需的 Web 产物）→ 生产构建 → electron-builder。
...
| 目录 / 文件                                        | 职责                                                           |
| -------------------------------------------------- | -------------------------------------------------------------- |
| `packages/desktop`                                 | Electron Main、Host、Renderer、内置手机端访问服务与桌面打包    |
