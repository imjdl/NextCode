# Spec：手机端访问（桌面端内置面板 + 局域网扫码，不支持公网）
...
2. **控制面板**：
   ...
   - 风险提示：该链接等于本机全权访问（可读写文件、执行命令）；服务只在局域网内可达，用完关闭。
...
- **可达范围仅限局域网**：不启用任何外网 relay/隧道，不做端口映射或内网穿透，也不托管到公网；
  手机必须与电脑处于同一网络（同一 Wi-Fi/网段）才能扫码访问。面板文案需明确写出这一限制。
