...
| 2026-09-21         | 3.16.1 | `ZCode-3.16.1-win-x64.exe`                 | 修复侧栏项目自动揭示失效 + 提示词增强点击报错；新增激活项目自动置顶、Web/手机端布局适配、桌面端手机端访问面板（见下）                                                                                                 |
...
| 2026-09-23         | 3.18.2 | `NextCode-3.18.2-win-x64.exe`              | **镜像抖动修复**：3.18.1 实测生成期间对话页每 ~2s 整页抖动——库级唤醒不分会话（空闲会话被误伤重水合）+ 整快照整替换引用。修复：refresh 内容指纹门控 + UI 快照按 rowId 保引用 + 合并告警降级（见 spec 第四阶段） |
...
### 2026-09-21 手机端访问（桌面端内置面板 + 局域网扫码，不支持公网）
...
```bash
node scripts/verify-desktop-artifact.mjs                  # 默认 win/x64，版本号读根 package.json
node scripts/verify-desktop-artifact.mjs --platform win --arch x64
```

它按版本号推导产物路径，逐项断言：安装包与 `latest.yml` 一致、可执行名为 `NextCode.exe`、
asar 内不含遥测 SDK（`@arms`）、CSP 与安全头就位（含 `'wasm-unsafe-eval'`）、品牌字形已换新
（新 N 在、旧 Z 不在）、手机端访问文案含「不支持公网访问」、已清理的死键与官方渠道文案不再出现、
随包远端资源与 Web 远控产物版本一致。失败时列出未通过项并以非零码退出。

**人工冒烟**：进 `dist/win-unpacked/` 直接运行 `NextCode.exe`，确认：启动徽标为 N、侧栏 footer 菜单只有
